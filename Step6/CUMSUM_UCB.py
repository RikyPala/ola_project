...
class CUMSUM_UCB_Matching(Learner):

    def __init__(self, n_arms, n_rows, n_cols, M = 100, eps = 0.05, h=20, alpha =0.01):
        super().__init__(n_arms, n_rows, n_cols)
        self.change_detection = [CUMSUM(M, eps, h) for _ in range(n_arms)] # each arm has a CUMSUM class
        self.valid_rewards_per_arms = [[] for _ in range(n_arms)]
        self.detections = [[] for _ in range(n_arms)]
        self.alpha = alpha  # the probability of picking the arm with the highest upper confidence bound
    ...
    def update(self, pulled_arms, rewards):
        ...
        for pulled_arm, reward in zip(pulled_arm_flat, rewards):
            if self.change_detection[pulled_arm].update(reward): # If this if is okay it means that there was an abrupt change, so we detect it and we reset all the CUMSUM parameters
