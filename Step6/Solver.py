...
class Solver:
    ...
    def __init__(self, env: NonStationaryEnvironment):
        ...
        self.n_phases = env.n_phases
        self.prices = env.prices
        self.conversion_rates = np.sum(env.conversion_rates * np.expand_dims(env.user_probabilities, axis=(1, 2)), axis=1)
    ...
    def find_optimal(self):
        ...
        for phase in range(self.n_phases):
            ...
            optimal_configurations.append(optimal_configuration)
        return optimal_configurations, optimal_rewards

    def compute_children_contribute(self, predecessors, configuration, i):
        root = predecessors[-1]
        contribute = 0
        for child, prob in enumerate(self.inverse_graph[root]):
            if prob == 0 or child in predecessors:
                continue
            contribute += prob * self.conversion_rates[i, child, configuration[child]] * (
                    self.expected_alpha_ratios[child] +
                    self.compute_children_contribute([*predecessors, child], configuration, i)
            )
        return contribute
