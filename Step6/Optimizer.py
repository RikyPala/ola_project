import numpy as np
from Non_Stationary_Environment import Non_Stationary_Environment, RoundData
from UCB import UCB


class Optimizer:

    def __init__(self, env: Non_Stationary_Environment, learner: UCB, window_size):
        ...
        self.conversion_rates_data = np.full((env.horizon, self.n_products, self.n_arms, 2), 0.)
    ...
    def update_conversion_rates(self, prod, price, visits, conversions):
        self.conversion_rates_data[self.t, prod, price, 0] += conversions
        self.conversion_rates_data[self.t,prod, price, 1] += visits
        if self.t > self.window_size:
            tau = self.t-self.window_size
        else:
            tau = 0
        self.conversion_rates_est[prod, price] = \
           np.sum(self.conversion_rates_data[tau:self.t+1,prod, price, 0])  / np.sum(self.conversion_rates_data[tau:self.t+1,prod, price, 1])
